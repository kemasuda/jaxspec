...
__all__ = ["air_to_vac", "compute_grid_coelho",
           "compute_grid_ispec", "compute_grid_bosz", "compute_grid_tlusty"]
...
def air_to_vac(wav_air_aa):
    ...
    n = 1 + 0.00008336624212083 + 0.02408926869968 / \
        (130.1065924522 - s*s) + 0.0001599740894897 / (38.92568793293 - s*s)
    return wav_air_aa * n
...
def compute_grid_coelho(model_params, wmin, wmax, data_dir, output_dir, fixed_wavgrid_length=5000, air_or_vac="vac", normalized=True):
    ...
    d = df_all.sort_values(
        ["teff", "logg", "feh", "alpha", "wav"]).reset_index(drop=True)
    ...
    print("wavelength resolution in the output grid: %d" %
          (wavarr[0]/np.diff(wavarr)[0]))

    pgrids2d = []
    for key in keys:
        pgrid2d = np.zeros(
            (len(tgrid), len(ggrid), len(fgrid), len(agrid), len(wavarr)))
    ...
    np.savez(outname, tgrid=tgrid, ggrid=ggrid, fgrid=fgrid,
             agrid=agrid, wavgrid=wavarr, flux=pgrids2d[0].astype(np.float32))
...
def mfmt(m): return "+%.2f" % (np.abs(m)) if m >= 0 else "-%.2f" % (np.abs(m))
...
def compute_grid_bosz(wminaa, wmaxaa, data_dir, output_dir, print_info=False, wavgrid_length=4000, air_or_vac="vac",
    ...
    for idx, (t, g, m, a, c, v) in enumerate(product(teff, logg, mh, alpha, carbon, vmic)):
        ...
        if g > 3.:
            filename = "bosz2024_mp_t%d_g+%.1f_m%s_a%s_c%s_v%d_rorig_noresam.txt.gz" % (
                t, g, mfmt(m), mfmt(a), mfmt(c), v)
        else:
            filename = "bosz2024_ms_t%d_g+%.1f_m%s_a%s_c%s_v%d_rorig_noresam.txt.gz" % (
                t, g, mfmt(m), mfmt(a), mfmt(c), v)
        try:
            di = read_single_spectrum(
                data_dir_m/filename, wminaa, wmaxaa, air_or_vac=air_or_vac)
        except:
            print(filename, "not found.")
            filename = "bosz2024_mp_t%d_g+%.1f_m%s_a%s_c%s_v%d_rorig_noresam.txt.gz" % (
                t, 4.0, mfmt(m), mfmt(a), mfmt(c), v)
            di = read_single_spectrum(
                data_dir_m/filename, wminaa, wmaxaa, air_or_vac=air_or_vac)
    ...
    d = df_all.sort_values(
        ["teff", "logg", "mh", "alpha", "carbon", "vmic", "wav"]).reset_index(drop=True)

    wavarr = np.linspace(np.max(wmin_all), np.min(wmax_all), wavgrid_length)
    print("# wavelength resolution in the output grid: %d" %
          (wavarr[0]/np.diff(wavarr)[0]))

    pgrid2d = np.zeros((len(teff), len(logg), len(mh), len(
        alpha), len(carbon), len(vmic), len(wavarr)))
                ...
                for l, a in enumerate(alpha):
                    for m, c in enumerate(carbon):
                        for n, v in enumerate(vmic):
                            _d = d[(d.teff == t) & (d.logg == g) & (d.mh == f) & (
                                d.alpha == a) & (d.carbon == c) & (d.vmic == v)]
                            pgrid2d[i][j][k][l][m][n] = interp1d(
                                _d.wav, _d["flux"])(wavarr)
    ...
    np.savez(outname, tgrid=teff, ggrid=logg, mgrid=mh, agrid=alpha,
             cgrid=carbon, vgrid=vmic, wavgrid=wavarr, flux=pgrid2d.astype(np.float32))
    print("# spectrum grid saved to %s." % outname)
    print()
...
def compute_grid_tlusty(wminaa, wmaxaa, data_dir, output_dir, print_info=False, wavgrid_length=30000, absolute=False):
    """TLUSTY grid for vmic=2km/s
            ...
            Args:
                    ...
                    data_dir: path to grid files
                    output_dir: output directory
                    wavgrid_length: length of wavelength grid. 30000 corresponds to ~2e4 for PFS

            Returns:
                    path to output file

    """
    print("# computing TLUSTY spectrum grid for %dAA-%dAA" % (wminaa, wmaxaa))
    ...
    for dirname in dirnames:
        ...
        for fname_cont, fname_spec in zip(files_cont, files_spec):
            ...
            idx = (wminaa < di["wav"]) & (di["wav"] < wmaxaa)
            di = di[idx].reset_index(drop=True)
            if absolute:
                di["flux"] = di["flux"] / np.median(di["flux"])
            else:
                di["flux"] = di["flux"] / func(di["wav"])
            di["teff"] = _teff
            di["logg"] = _logg
            di["logZ"] = _logZ
            # di["vmic"] = _vmic
            ...
            # vmic.append(_vmic)
            if print_info:
                print("median wavres:", np.median(
                    di.wav[1:] / np.diff(di.wav)))

    df_all = pd.concat(df_all).reset_index(drop=True)
    d = df_all.sort_values(
        ["teff", "logg", "logZ", "wav"]).reset_index(drop=True)

    wavarr = np.linspace(np.max(wmin_all), np.min(wmax_all), wavgrid_length)
    print("# wavelength resolution in the output grid: %d" %
          (wavarr[0]/np.diff(wavarr)[0]))

    teff, logg, logZ = np.sort(np.array(list(set(teff)))),  np.sort(np.array(list(
        set(logg)))), np.sort(np.array(list(set(logZ))))
    logg = logg[logg > 2.9]  # some don't have low logg values
    print(teff, logg, logZ)

    pgrid2d = np.zeros(
        (len(teff), len(logg), len(logZ), len(wavarr)))
    for i, t in enumerate(teff):
        for j, g in enumerate(logg):
    ...
    if absolute:
        outname = output_dir / Path("%d-%d_abs.npz" % (wminaa, wmaxaa))
    else:
        outname = output_dir / Path("%d-%d_normed.npz" % (wminaa, wmaxaa))
    if not os.path.exists(output_dir):
        Path(output_dir).mkdir()

    np.savez(outname, tgrid=teff, ggrid=logg, zgrid=logZ,
             wavgrid=wavarr, flux=pgrid2d.astype(np.float32))
    print("# spectrum grid saved to %s." % outname)
    print()
