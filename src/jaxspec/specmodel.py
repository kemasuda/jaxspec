...
class SpecModel:
    ...
    def fluxmodel(self, wav_out, par):
        ...
        flux_base = c0 + c1 * (wav_out - jnp.mean(self.wav_obs, axis=1)
                               [:, jnp.newaxis]) / self.wav_obs_range[:, jnp.newaxis]
        flux_phys = flux_base * ((1 - dilution) * broaden_and_shift_vmap(wav_out, self.wavgrid,
                                 flux_raw, vsini, zeta, get_beta(wavres), rv, self.varr, u1, u2) + dilution)
        return flux_phys
    ...
    def fluxmodel_multiorder(self, par):
        ...
        if self.sg.model == 'bosz':
            flux_raw = self.sg.values(
                teff, logg, feh, alpha, par['carbon'], par['vmic'], self.wavgrid)
        else:
            flux_raw = self.sg.values(teff, logg, feh, alpha, self.wavgrid)
        ...
        for j in range(len(flux_model)):
            ...
            if not self.gpu:
                ...
                gp = tinygp.GaussianProcess(
                    kernel, self.wav_obs[j][idxj], diag=diags[j][idxj], mean=0.0)
...
class SpecModel2(SpecModel):
    ...
    def fluxmodel(self, wav_out, params_phys):
        ...
        flux_sum = broaden_and_shift_vmap(wav_out, self.wavgrid, flux_raw1, vsini1, zeta1, get_beta(wavres), rv1, self.varr, u1, u2) + \
            f2_f1 * broaden_and_shift_vmap(wav_out, self.wavgrid, flux_raw2,
                                           vsini2, zeta2, get_beta(wavres), rv2, self.varr, u1, u2)
        flux_base = c0 + c1 * (wav_out - jnp.mean(self.wav_obs, axis=1)
                               [:, jnp.newaxis]) / self.wav_obs_range[:, jnp.newaxis]
    ...
    def fluxmodel_multiorder(self, c0, c1, teff1, teff2, logg1, logg2, feh1, feh2,
        ...
        flux_sum = broaden_and_shift_vmap_full(wav_out, self.wavgrid, flux_raw1, vsini1, zeta1, get_beta(
            res), rv1, self.varr, u1, u2) + f2_f1 * broaden_and_shift_vmap_full(wav_out, self.wavgrid, flux_raw2, vsini2, zeta2, get_beta(res), rv2, self.varr, u1, u2)
        flux_phys = flux_base * flux_sum / (1. + f2_f1)
        return flux_phys
