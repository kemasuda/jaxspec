...
def rotkerneld(varr, zeta, vsini, u1, u2, beta, Nt=1000, Ns=10000, smax=10.):
    tarr = jnp.linspace(0, 1, Nt)
    sigmas = jnp.linspace(-smax, smax, Ns)
    ...
    ret = jnp.trapz(ys.T)#, x=tarr)

    freqs = jnp.fft.fftfreq(len(ret)) / jnp.diff(sigmas)[0]
    kernel = jnp.abs(jnp.fft.fft(ret))
    idx = jnp.argsort(freqs)
    freqs, kernel = freqs[idx], kernel[idx]

    return jnp.interp(varr, freqs, kernel)

#%% use aymmetric varr with odd len(varr)
def rotkernel(varr, zeta, vsini, u1, u2, beta, Nt=1000):
    tarr = jnp.linspace(0, 1, Nt)
